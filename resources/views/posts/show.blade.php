...
@section('content')
      ...
      <h1 class="mt-4">{{$post->title}}</h1>
        ...
        <form method="POST" action="{{route('posts.like',['post'=>$post->id])}}">
          ...
          @if($post->likedBy()->where('user_id',Auth::id())->count()== 1)

            <p>You @if ($post->likedBy()->count() > 1)
              and {{$post->likedBy()->count() - 1}}
            @endif  
              like this</p>

            @else
              <p> a {{$post->likedBy()->count()}} personas le gustaron el post</p>
